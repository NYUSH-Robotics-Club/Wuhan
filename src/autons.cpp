...
// autoIsRed = True means we are sorting OUT blue, meaning we are on the red
// alliance.
...
void redAuto() {
  ...
  intakeMotors.spin(reverse, 12, volt);
  wait(.3, seconds);
  intakeMotors.spin(forward, 12, volt);
  ...
  waitUntil(
      ...
      (conveyor.position(degrees) == autoConveyorPosition + 3000));
  intakeMotors.stop();
...
void holonomic_odom_test() {
  ...
  // This is just a test (that didn't work), no real need to keep it if we need
  // the auton Drive to Point test
  default_constants();
  inertialSensor.calibrate();
  wait(2000, msec);
  Controller1.rumble("--");
